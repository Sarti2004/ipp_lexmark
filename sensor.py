...
async def async_setup_entry(
    ...
    sensors.append(IPPErrorSensor(entry.entry_id, unique_id, coordinator))

    _LOGGER.debug(f"ipp data {coordinator.data}")
...
class IPPErrorSensor(IPPSensor):
    """Defines an IPP printer sensor."""
    ...
    def __init__(
        ...
        super().__init__(
            ...
            icon="mdi:alert",
            key="msg_full",
            name=f"{coordinator.data.info.name} Errors",
            unit_of_measurement=None,
            entity_category=EntityCategory.DIAGNOSTIC
        )
    ...
    def native_value(self) -> str:
        """Return the state of the sensor."""
        return self.coordinator.data.state.message_full
...
class IPPUptimeSensor(IPPSensor):
    ...
    def native_value(self) -> int:
        """Return the state of the sensor."""
        _LOGGER.debug(f"ipp uptime {self.coordinator.data.info}")

        #return utcnow() - timedelta(seconds=int(self.coordinator.data.info.uptime))
        return self.coordinator.data.state.uptime
